...
class AdminRoleSeeder extends Seeder
{
    public function run(): void
    {
        $owner = AdminRole::updateOrCreate(
            ['slug' => 'owner'],
            ['name' => 'Owner (Super Admin)', 'is_super' => true]
        );

        $content = AdminRole::updateOrCreate(
            ['slug' => 'content_admin'],
            ['name' => 'Admin Konten', 'is_super' => false]
        );

        $catalog = AdminRole::updateOrCreate(
            ['slug' => 'catalog_admin'],
            ['name' => 'Admin Produk', 'is_super' => false]
        );

        $orders = AdminRole::updateOrCreate(
            ['slug' => 'order_admin'],
            ['name' => 'Admin Order', 'is_super' => false]
        );

        $finance = AdminRole::updateOrCreate(
            ['slug' => 'finance_admin'],
            ['name' => 'Admin Finance', 'is_super' => false]
        );

        $marketing = AdminRole::updateOrCreate(
            ['slug' => 'marketing_admin'],
            ['name' => 'Admin Marketing', 'is_super' => false]
        );

        $auditor = AdminRole::updateOrCreate(
            ['slug' => 'auditor'],
            ['name' => 'Auditor (Read Only)', 'is_super' => false]
        );
        ...
        // Owner is_super = true => allow all (tidak perlu sync)
    }
}
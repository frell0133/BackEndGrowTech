...
class UserDeliveryController extends Controller
{
    ...
    public function info(Request $request, $id, OrderFulfillmentService $fulfill)
    {
        ...
        return $this->ok([
            ...
            'gateway_fee_amount' => (string) ($order->gateway_fee_amount ?? '0'),
            'total_payable_gateway' => (string) ((float) ($order->amount ?? 0) + (float) ($order->gateway_fee_amount ?? 0)),
            ...
            'items' => $order->items->map(fn ($item) => [
                ...
                'product_slug' => (string) ($item->product_slug ?? $item->product?->slug ?? ''),
                'qty' => (int) ($item->qty ?? 1),
